...
inline __host__ __device__ int4 operator+(int4 x, int4 y){
    return make_int4(x.x + y.x, x.y + y.y, x.z + y.z, x.w + y.w);
}
...
inline  __host__ __device__ int min(int2 v){
    return min(v.x, v.y);
}
inline  __host__ __device__ int min(int3 v){
    return min(v.x, min(v.y, v.z));
}
inline  __host__ __device__ int min(int4 v){
...
inline  __host__ __device__ int max(int2 v){
    return max(v.x, v.y);
}
inline  __host__ __device__ int max(int3 v){
    return max(v.x, max(v.y, v.z));
}
inline  __host__ __device__ int max(int4 v){
