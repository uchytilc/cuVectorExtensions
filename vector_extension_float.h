...
inline __host__ __device__ uint3 operator>(float3 a, float b) {
...
inline __host__ __device__ uint3 operator>(float3 a, float3 b) {
...
inline  __host__ __device__ float2 min(float x, float2 y){
    return make_float2(fminf(x,y.x), fminf(x,y.y));
}
inline  __host__ __device__ float2 min(float2 x, float y){
    return make_float2(fminf(x.x,y), fminf(x.y,y));
}
inline  __host__ __device__ float2 min(float2 x, float2 y){
    return make_float2(fminf(x.x,y.x), fminf(x.y,y.y));
}
inline __host__ __device__ float3 min(float x, float3 y){
    return make_float3(fminf(x,y.x), fminf(x,y.y), fminf(x,y.z));
}
inline __host__ __device__ float3 min(float3 x, float y){
    return make_float3(fminf(x.x,y), fminf(x.y,y), fminf(x.z,y));
}
inline __host__ __device__ float3 min(float3 x, float3 y){
    return make_float3(fminf(x.x,y.x), fminf(x.y,y.y), fminf(x.z,y.z));
}
inline  __host__ __device__ float4 min(float x, float4 y){
    return make_float4(fminf(x,y.x), fminf(x,y.y), fminf(x,y.z), fminf(x,y.w));
}
inline  __host__ __device__ float4 min(float4 x, float y){
    return make_float4(fminf(x.x,y), fminf(x.y,y), fminf(x.z,y), fminf(x.w,y));
}
inline  __host__ __device__ float4 min(float4 x, float4 y){
    return make_float4(fminf(x.x,y.x), fminf(x.y,y.y), fminf(x.z,y.z), fminf(x.w,y.w));
}

inline  __host__ __device__ float min(float2 v){
    return min(v.x, v.y);
}
inline  __host__ __device__ float min(float3 v){
    return min(v.x, min(v.y, v.z));
}
inline  __host__ __device__ float min(float4 v){
...
inline  __host__ __device__ float2 max(float x, float2 y){
    return make_float2(fmaxf(x,y.x), fmaxf(x,y.y));
}
inline  __host__ __device__ float2 max(float2 x, float y){
    return make_float2(fmaxf(x.x,y), fmaxf(x.y,y));
}
inline  __host__ __device__ float2 max(float2 x, float2 y){
    return make_float2(fmaxf(x.x,y.x), fmaxf(x.y,y.y));
}
inline __host__ __device__ float3 max(float x, float3 y){
    return make_float3(fmaxf(x,y.x), fmaxf(x,y.y), fmaxf(x,y.z));
}
inline __host__ __device__ float3 max(float3 x, float y){
    return make_float3(fmaxf(x.x,y), fmaxf(x.y,y), fmaxf(x.z,y));
}
inline __host__ __device__ float3 max(float3 x, float3 y){
    return make_float3(fmaxf(x.x,y.x), fmaxf(x.y,y.y), fmaxf(x.z,y.z));
}
inline  __host__ __device__ float4 max(float x, float4 y){
    return make_float4(fmaxf(x,y.x), fmaxf(x,y.y), fmaxf(x,y.z), fmaxf(x,y.w));
}
inline  __host__ __device__ float4 max(float4 x, float y){
    return make_float4(fmaxf(x.x,y), fmaxf(x.y,y), fmaxf(x.z,y), fmaxf(x.w,y));
}
inline  __host__ __device__ float4 max(float4 x, float4 y){
    return make_float4(fmaxf(x.x,y.x), fmaxf(x.y,y.y), fmaxf(x.z,y.z), fmaxf(x.w,y.w));
}

inline  __host__ __device__ float max(float2 v){
    return max(v.x, v.y);
}
inline  __host__ __device__ float max(float3 v){
    return max(v.x, max(v.y, v.z));
}
inline  __host__ __device__ float max(float4 v){
...
inline __device__ __host__ float3 clamp(float3 v, float lo, float3 hi){
    return make_float3(clamp(v.x, lo, hi.x), clamp(v.y, lo, hi.y), clamp(v.z, lo, hi.z));
}
inline __device__ __host__ float3 clamp(float3 v, float3 lo, float hi){
    return make_float3(clamp(v.x, lo.x, hi), clamp(v.y, lo.y, hi), clamp(v.z, lo.z, hi));
}
...
inline __device__ __host__ float param(float o, float d, float t)
{
    return o + d*t;
}
inline __device__ __host__ float2 param(float2 o, float2 d, float t)
{
    return o + d*t;
}
inline __device__ __host__ float3 param(float3 o, float3 d, float t)
{
    return o + d*t;
}
inline __device__ __host__ float4 param(float4 o, float4 d, float t)
{
    return o + d*t;
}
